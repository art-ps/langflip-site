...
3. прогоните `npm test`.

Версия попадает и в текст страницы, и в JSON-LD `SoftwareApplication` — из одного места. Тесты сверяют `release` с файлом на диске (имя, формат версии, размер), поэтому расхождение между кнопкой и тем, что реально лежит в `public/`, роняет сборку — а сами тесты при смене версии править не нужно.
